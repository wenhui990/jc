...
function luckydrawCallBack(dt){
	...
	if(dt){
		...
		userDiv+='<ul class="list_con2" id="dynamicshow" style="width:100%">';
...
//				}
				...
				var str = {"1":5428,"2":5530,"3":5942,"4":5270,"5":5686,"6":5734,"0":5850};
				$("#run").rotate({ 
					duration:3000, //转动时间 
					angle: 0, //默认角度
...
//		}
		win = ((approvalRate["3"]/approvalRateSums)*100).toFixed(2);
		draw = ((approvalRate["1"]/approvalRateSums)*100).toFixed(2);
...
// 	 				td.width="260";
 	 				if(dt[i].name&&dt[i].worth){
 	 					if(dt[i].num){
 	 						td.innerHTML = '<button class="btn erweima" style="background: #1a569d;padding: 2px 5px;" data-id="'+dt[i].name+dt[i].code+'">二维码</button>';
 	 					}else{
 	 						td.innerHTML = "发奖中";
 	 					}
 	 				}else{
